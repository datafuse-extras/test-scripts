        ...
        tokio::spawn({
            ...
            async move {
                ...
                for batch_id in 0..iterations {
                    ...
                    if success {
                        num_of_success += 1;
                        // only if replace_into is successful, we execute replace_conflict
                        if batch_id %  7 == 0 {
                            let mut ids = Vec::new();
                            ids.push(batch_id);
                            if batch_id > 5 {
...
async fn exec_replace_conflict(dsn: &str, batch_ids: &[u32]) -> Result<bool> {
    ...
    let sub_queries =
       batch_ids.iter().map(|id| format!("select * from test_order where id1 = {}", id)).collect::<Vec<_>>().join(" union all ");
...
async fn verify(dsn: &str, success_replace_stmts: u32) -> Result<()> {
    ...
    info!("                           ");
    info!("                           ");
    info!("number of successfully executed replace-into statements : {}", success_replace_stmts);
    info!("                           ");
    info!("                           ");
